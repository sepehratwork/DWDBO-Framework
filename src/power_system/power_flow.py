...
def _eval_timestep_flow_worker(args: Tuple[int, np.ndarray, np.ndarray, float, float, np.ndarray, Any]) -> Tuple[int, float, float]:
    ...
    # Renewable injection at bus 12 (0-indexed 12 = Bus 13)
    res_bus = 12
    P_inj[res_bus] += (P_RES_t - P_curt_t)
    ...
    # Load demand distributed proportional to IEEE 30-bus load profile
    P_inj -= (P_D[t] * sys_data.bus_load_weights)
...
class MultiPeriodOPFSolver:
    ...
    def solve_multi_period_dispatch(
        ...
        """
        Solves multi-period OPF minimization problem (Eq. 10 - Eq. 15) using optimization.
        ...
        def objective(x: np.ndarray) -> float:
            ...
            P_ch = x[idx_pch : idx_pdis].reshape((T, num_bess)) if num_bess > 0 else np.zeros((T, 0))
            P_dis = x[idx_pdis : idx_soc].reshape((T, num_bess)) if num_bess > 0 else np.zeros((T, 0))

            cost_gen = np.sum(
                self.sys.cost_a * (P_g ** 2) + self.sys.cost_b * P_g + self.sys.cost_c
            )
            cost_bess = np.sum(self.bess_cfg.degradation_cost * (P_ch + P_dis)) if num_bess > 0 else 0.0
        ...
        for t in range(T):
            def make_balance_rule(time_step):
                def balance_rule(x: np.ndarray) -> float:
                    ...
                    pch_t = x[idx_pch + time_step * num_bess : idx_pch + (time_step + 1) * num_bess] if num_bess > 0 else np.array([0.0])
                    pdis_t = x[idx_pdis + time_step * num_bess : idx_pdis + (time_step + 1) * num_bess] if num_bess > 0 else np.array([0.0])
                    p_curt_t = x[idx_curtail + time_step]

                    gen_total = np.sum(pg_t)
                    bess_net = np.sum(pdis_t - pch_t) if num_bess > 0 else 0.0
                    res_net = P_RES[time_step] - p_curt_t
                    loss_est = 0.015 * P_D[time_step]
        ...
        dt = 1.0
        for b in range(num_bess):
            cap_b = max(1e-3, bess_capacities[b])
            for t in range(T):
        ...
        for t in range(T):
            for b in range(num_bess):
                p_rate = min(self.bess_cfg.power_max_mw, max(0.0, bess_capacities[b] * 0.25))
                bounds.append((0.0, p_rate))
        # P_dis bounds
        for t in range(T):
            for b in range(num_bess):
                p_rate = min(self.bess_cfg.power_max_mw, max(0.0, bess_capacities[b] * 0.25))
        ...
        # Capital investment cost normalized to daily scheduling
        c_inv = float(np.sum(bess_capacities) * 0.0568) if num_bess > 0 else 0.0

        P_g_opt = x_opt[idx_pg : idx_pch].reshape((T, num_gen))
        P_curt_opt = x_opt[idx_curtail :]

        # Multi-Processing evaluation for Network Power Flow & Losses across timesteps with progress display
        worker_args = [
        ...
        v_dev_results = [0.0] * T
        l_loss_results = [0.0] * T

        if self.n_workers > 1:
            with ProcessPoolExecutor(max_workers=self.n_workers) as executor:
                futures = [executor.submit(_eval_timestep_flow_worker, arg) for arg in worker_args]
                for future in as_completed(futures):
                    t_idx, v_dev, l_loss = future.result()
                    v_dev_results[t_idx] = v_dev
                    l_loss_results[t_idx] = l_loss
        else:
            for arg in worker_args:
                t_idx, v_dev, l_loss = _eval_timestep_flow_worker(arg)
                v_dev_results[t_idx] = v_dev
                l_loss_results[t_idx] = l_loss

        v_dev_avg = float(sum(v_dev_results) / T)
        l_loss_total = float(sum(l_loss_results))
        curtailment_pct = float((np.sum(P_curt_opt) / max(1e-5, np.sum(P_RES))) * 100.0)

        commitment_matrix = (P_g_opt > (self.sys.p_min * 1.05)).astype(int).T
